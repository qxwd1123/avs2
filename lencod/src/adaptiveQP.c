 ...
 * \param uiMaxAQDepth Maximum depth of unit block for assigning QP adaptive to
 * local image characteristics \return void
 */
void adaptiveQPInit(int iWidth, int iHeight, unsigned int uiMaxAQDepth) {
  ...
  if (uiMaxAQDepth > 0) {
    aQPpic->m_acAQLayer =
        (PicAdaptiveQLayer *)calloc(uiMaxAQDepth, sizeof(PicAdaptiveQLayer));
    for (d = 0; d < uiMaxAQDepth; d++) {
      ...
      aQPpic->m_acAQLayer[d].m_acTEncAQU = (AdaptiveQUnit *)calloc(
          uiNumAQPartInWidth * uiNumAQPartInHeight, sizeof(AdaptiveQUnit));
    }
  }
}

/** Analyze source picture and compute local image characteristics used for QP
 * adaptation \param iWidth Picture width \param iHeight Picture height \return
 * Void
 */
  ...
  for (d = 0; d < aQPpic->m_uiMaxAQDepth; d++) {
    ...
    for (y = 0; y < iHeight; y += uiAQPartHeight) {
      ...
      for (x = 0; x < iWidth; x += uiAQPartWidth, pcAQU++) {
        ...
        for (i = 0; i < 4; i++) {
          ...
          dVariance =
              (double)(uiSumSq[i]) / uiNumPixInAQPart - dAverage * dAverage;
    ...
    for (aquy = 0; aquy < uiNumAQPartInHeight; aquy++) {
      for (aqux = 0; aqux < uiNumAQPartInWidth; aqux++, pcAQU++) {
        pcAQU->dNormAct = (2.0 * pcAQU->dActivity + dAvgAct) /
                          (pcAQU->dActivity + 2.0 * dAvgAct);
      }
    }
  }
}
